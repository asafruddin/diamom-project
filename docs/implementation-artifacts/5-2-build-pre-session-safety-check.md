...
1. **Given** the user starts a session  
...
- [ ] Confirm story scope and existing implementation (AC: all)
  ...
  - [ ] Inspect existing files before editing, especially app/session/safety-check.tsx, src/constants/safety.ts, src/features/session/.
...
- [ ] Implement Build Pre-Session Safety Check (AC: 1)
...
- `docs/planning-artifacts/epics.md` Story 5.2 is the source of truth for the story statement and acceptance criteria.
...
- Previous available story key: `5-1-implement-session-state-engine-and-route-guards`.
