...
1. **Given** the session flow starts  
...
- [ ] Confirm story scope and existing implementation (AC: all)
  ...
  - [ ] Inspect existing files before editing, especially src/stores/session.store.ts, src/services/session.service.ts, src/features/session/session-state.ts.
...
- [ ] Implement Implement Session State Engine and Route Guards (AC: 1)
...
**Safety-Gated Guided Session Flow** — Users can complete a full safety-gated guided practice loop: pre-session check, VAS before, session selection, preparation, guided timer, pause/stop/emergency handling, emergency contact access, VAS after, and high-VAS safety guidance.

Session safety story: enforce deterministic state transitions, route guards, VAS requirements, pause/stop/emergency behavior, and no bypasses.

### Source Requirements

- `docs/planning-artifacts/epics.md` Story 5.1 is the source of truth for the story statement and acceptance criteria.
...
- `src/features/session/session-state.ts`
...
- Previous available story key: `4-6-connect-learning-content-to-safe-practice-entry`.
