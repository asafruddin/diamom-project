...
1. **Given** a session produces before/after VAS data or incomplete result data  
...
- [ ] Confirm story scope and existing implementation (AC: all)
  ...
  - [ ] Inspect existing files before editing, especially src/db/client.ts, src/db/schema.ts, src/db/repositories/, src/services/vas.service.ts.
...
- [ ] Implement Implement Local Session and VAS Persistence (AC: 1)
...
**Results, Local History, and VAS Progress** — Users can save session outcomes locally, review activity history, inspect session details, delete records, and view VAS trends without the app making medical effectiveness claims.

Local records story: use SQLite/repositories for persistence and keep VAS/history language claim-safe.

### Source Requirements

- `docs/planning-artifacts/epics.md` Story 6.1 is the source of truth for the story statement and acceptance criteria.
...
- Previous available story key: `5-9-apply-high-vas-safety-guidance`.
