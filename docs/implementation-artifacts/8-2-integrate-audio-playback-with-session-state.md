...
1. **Given** audio preferences are enabled  
...
- [ ] Confirm story scope and existing implementation (AC: all)
  ...
  - [ ] Inspect existing files before editing, especially src/lib/audio.ts, src/services/session.service.ts, src/stores/session.store.ts.
...
- [ ] Implement Integrate Audio Playback With Session State (AC: 1)
...
- `docs/planning-artifacts/epics.md` Story 8.2 is the source of truth for the story statement and acceptance criteria.
...
- Previous available story key: `8-1-build-voice-guide-and-music-settings`.
