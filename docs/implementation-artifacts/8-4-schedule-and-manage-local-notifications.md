...
1. **Given** reminder settings are saved  
...
- [ ] Confirm story scope and existing implementation (AC: all)
  ...
  - [ ] Inspect existing files before editing, especially src/lib/notifications.ts, src/db/repositories/notification-reminders.repository.ts.
...
- [ ] Implement Schedule and Manage Local Notifications (AC: 1)
...
**Supportive Engagement: Voice Guidance, Music, and Reminders** — Users can configure voice guidance, background music, volume, instruction speed, and gentle practice/hydration reminders to make sessions easier to follow and return to.

Engagement story: keep reminders/audio optional, local, and synchronized with session state.

### Source Requirements

- `docs/planning-artifacts/epics.md` Story 8.4 is the source of truth for the story statement and acceptance criteria.
...
- `src/db/repositories/notification-reminders.repository.ts`
...
- Previous available story key: `8-3-build-reminder-permission-and-settings`.
