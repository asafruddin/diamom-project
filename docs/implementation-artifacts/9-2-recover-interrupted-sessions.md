...
1. **Given** the app exits, a phone call interrupts, or the session is otherwise interrupted  
...
- [ ] Confirm story scope and existing implementation (AC: all)
  ...
  - [ ] Inspect existing files before editing, especially src/services/session.service.ts, src/stores/session.store.ts.
...
- [ ] Implement Recover Interrupted Sessions (AC: 1)
...
- `docs/planning-artifacts/epics.md` Story 9.2 is the source of truth for the story statement and acceptance criteria.
...
- Previous available story key: `9-1-implement-shared-empty-and-error-states`.
